...
def train(_config, train_dataset, val_dataset, test_dataset):
    ...
    if optimizer_mask:
        optimizer_mask = fabric.setup_optimizers(optimizer_mask)
    steps = 0
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in tqdm(
            ...
            pred, pred_mask = model(image, question, mask)
            answer = answer
            mae = F.l1_loss(mask, pred_mask)
            mse = F.mse_loss(mask, pred_mask)
            rmse = torch.sqrt(mse)
            ...
            steps += 1
            accLoss += step_acc * image.shape[0] * num_ddp
            maeLoss += step_mae * image.shape[0] * num_ddp
            rmseLoss += step_rmse * image.shape[0] * num_ddp
            # --------------------- L1 ---------------------------
            if _config["L1Reg"]:
                L1_reg = 0
            ...
            if (i + 1) % miniStep == 0:
                if opts and epoch < _config["thread_epoch"]:
                    optimizer_mask.step()
        ...
        with torch.no_grad():
            ...
            for i, data in tqdm(
                ...
                if not _config['normalize']:
                    mae = mae / 255
                    rmse = rmse / 255
                accLoss += acc_loss.cpu().item() * image.shape[0] * num_ddp
                maeLoss += mae.cpu().item() * image.shape[0] * num_ddp
                rmseLoss += rmse.cpu().item() * image.shape[0] * num_ddp
            ...
            if valLoss[epoch] < bestVal:
                ...
                torch.save(model.state_dict(), f"{saveDir}mask_bestValLoss.pth")
            logger.info(
