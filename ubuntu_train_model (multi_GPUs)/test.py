...
def test_model(_config, model, test_loader, test_length, logger, wandb_epoch=None, epoch=0, ddp=1):
    v1 = time.time()
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(
            ...
            if not _config['normalize']:
                mae = mae / 255
                rmse = rmse / 255
            accLoss += acc_loss.cpu().item() * image.shape[0] * ddp
            maeLoss += mae.cpu().item() * image.shape[0] * ddp
            rmseLoss += rmse.cpu().item() * image.shape[0] * ddp
...
def main(_config):
    ...
    state_dict = torch.load(saveDir + modelName)
    ...
    test_model(_config, model, test_loader, test_length, logger)
